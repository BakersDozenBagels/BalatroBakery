...
Bakery_API.credit(SMODS.Consumable({
	...
	can_use = function(self, card)
		...
		return count >= 1
			and count <= card.ability.extra.hl
			and (
				#Bakery_API.get_highlighted() > 0
				or #G.jokers.highlighted + #G.jokers.cards <= G.jokers.config.card_limit
			)
	end,
	use = function(self, card, area, copier)
			...
			func = function()
				...
				for i = 1, #Bakery_API.get_highlighted() do
					...
					for i = 1, card.ability.extra.copies do
						...
						local hand_available = G.TAROT_INTERRUPT == G.STATES.SELECTING_HAND
							or G.TAROT_INTERRUPT == G.STATES.TAROT_PACK
							or G.TAROT_INTERRUPT == G.STATES.SPECTRAL_PACK
							or G.TAROT_INTERRUPT == G.STATES.SMODS_BOOSTER_OPENED;
						(hand_available and G.hand or G.deck):emplace(_card)
	...
	use = function(self, card, area, copier)
		...
		for i = 1, #Bakery_API.get_highlighted() do
			...
			G.E_MANAGER:add_event(Event({
				...
				delay = 0.15,
				func = function()
					Bakery_API.get_highlighted()[i]:flip()
					play_sound("card1", percent)
					Bakery_API.get_highlighted()[i]:juice_up(0.3, 0.3)
					return true
				end,
			}))
		end
