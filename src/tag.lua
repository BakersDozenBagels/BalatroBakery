...
for i, t in ipairs(enhancement_tags) do
	SMODS.Tag({
		...
		loc_vars = function(self, info_queue, tag)
			...
			return {
				vars = {
					tag.ability.amount or self.config.amount,
					localize({ set = "Enhanced", type = "name_text", key = self.config.enhancement }),
				},
			}
		end,
		apply = function(self, tag, context)
			...
			if
				not tag.triggered
				and (tag.ability.amount or self.config.amount) > 0
				and context.type == "Bakery_score_card"
				and enhancement_tag_buffer ~= context.card
			then
