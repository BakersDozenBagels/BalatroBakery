...
Bakery_API.Joker({
	...
	calculate = function(self, card, context)
		...
		then
			...
			if
				rank > 0 -- Stone cards are random negative ranks
...
Bakery_API.Joker({
	...
	calculate = function(self, card, context)
		...
		if context.end_of_round and not context.repetition and context.game_over == false and not context.blueprint then
			if
				(card.ability.extra.flipped and card.ability.extra.discards >= 2)
				or (not card.ability.extra.flipped and card.ability.extra.discards == 0)
			then
...
function Bakery_API.get_proxied_joker()
	if G.jokers and G.jokers.cards then
		...
		for _, other in pairs(G.jokers.cards) do
			if
				other.config.center ~= G.P_CENTERS.j_Bakery_Proxy
				and other.ability.Bakery_purchase_index
				and other.ability.Bakery_purchase_index > latest
...
Bakery_API.Joker({
	...
	calculate = function(self, card, context)
		...
		if
			context.before
			and Bakery_API.big(G.GAME.hands[context.scoring_name].level) == Bakery_API.big(1)
			and not context.blueprint
		then
...
Bakery_API.Joker({
	...
	calculate = function(self, card, context)
		if context.joker_main then
			if
				not SMODS.has_no_suit(context.scoring_hand[1])
				and not SMODS.has_no_suit(context.scoring_hand[#context.scoring_hand])
				and (
					SMODS.has_any_suit(context.scoring_hand[1])
					or SMODS.has_any_suit(context.scoring_hand[#context.scoring_hand])
					or not context.scoring_hand[1]:is_suit(context.scoring_hand[#context.scoring_hand].base.suit)
				)
			then
				return {
...
Bakery_API.Joker({
	...
	Bakery_can_use = function(self, card)
		return Bakery_API.default_can_use(card)
			and Bakery_API.big(card.ability.extra.cost)
				<= Bakery_API.big(G.GAME.dollars) + Bakery_API.big(G.GAME.dollar_buffer or 0) - Bakery_API.big(
					G.GAME.bankrupt_at
				)
	end,
...
Bakery_API.guard(function()
	...
	function Bakery_API.alternates_suits(hand, first, second)
		if not first then
			return Bakery_API.alternates_suits(hand, Bakery_API.red_suits, Bakery_API.black_suits)
				or Bakery_API.alternates_suits(hand, Bakery_API.black_suits, Bakery_API.red_suits)
		end
...
Bakery_API.Joker({
	...
	calculate = function(self, card, context)
		...
		if context.joker_main then
			return card.ability.extra.flipped and { x_mult = card.ability.extra.x_mult }
				or { mult = card.ability.extra.mult }
		end
...
Bakery_API.Joker({
	...
	calculate = function(self, card, context)
		if
			context.individual
			and context.cardarea == G.play
			and context.other_card:get_id() == G.GAME.current_round.Bakery_Wherewolf_card.id
			and context.other_card:is_suit(G.GAME.current_round.Bakery_Wherewolf_card.suit)
		then
			if not context.blueprint then
				card.ability.extra.played_cards = true
			end
...
Bakery_API.Joker({
	...
	calculate = function(self, card, context)
		...
		if
			...
			and context.main_eval
			and card.ability.extra.flipped
			and G.GAME.chips / G.GAME.blind.chips * 100 >= Bakery_API.big(card.ability.extra.percent)
		then
			Bakery_API.flip_double_sided(card)
			return {
